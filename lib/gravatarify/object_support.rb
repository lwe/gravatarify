...
module Gravatarify::ObjectSupport
  ...
  alias_method :gravatarify_url, :gravatar_url
  ...
  module ClassMethods    
    def gravatarify(*args)
      ...
      define_method(:gravatar_url) do |*params|
        ...
        gravatarify_url send(source || :email), options.merge(params.first || {})
      end
      # has more
      args.each do |src|
        method = "#{src}_gravatar_url".sub(/_e?mail/, '')
        define_method(method) do |*params|
          gravatarify_url send(src), options.merge(params.first || {})
        end
      end
    end
  end
